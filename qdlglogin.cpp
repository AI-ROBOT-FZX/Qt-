...
void QDlgLogin::readSettings()
{
    ...
    QString appName="mycom_video"; //HKEY_CURRENT_USER/Software/WWB-Qt/amp6_5
...
{
    QSettings   settings("WWB-Qt","mycom_video"); //注册表键组
...
void QDlgLogin::mouseMoveEvent(QMouseEvent *event)//鼠标按下左键移动
{
...
//ui为QDlgLogin的对象，且QDlgLogin继承于 QDialog
...
void QDlgLogin::on_btnOK_clicked()
{
    ...
    else
    {
        ...
        if (m_tryCount>3)
        {
            QMessageBox::critical(this, "错误", "用户名或密码错误次数过多，请稍后再试");
            this->reject(); //退出
        }
        else
